 ...
 * @author Hxkandwal
 */
public class SingleRowKeyboard extends AbstractCustomTestRunner {

    public int _calculateTime(String keyboard, String word) {
        int[] map = new int [256];
        for (int idx = 0; idx < keyboard.length(); idx ++) map [keyboard.charAt(idx)] = idx;

        int ans = 0;
        for (int prev = -1, idx = 0; idx < word.length(); idx ++) {
            char c = word.charAt(idx);
            ans += Math.abs (map [c] - (prev == -1 ? 0 : map [prev]));
            prev = c;
        }

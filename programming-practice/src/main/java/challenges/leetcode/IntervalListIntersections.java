 ...
 * @author Hxkandwal
 */
public class IntervalListIntersections extends AbstractCustomTestRunner {
    ...
    public Interval[] _intervalIntersection(Interval[] A, Interval[] B) {
        int ai = 0, bi = 0;
        List<Interval> ans = new ArrayList<>();
        while (ai < A.length && bi < B.length) {
            Interval a = A [ai], b = B [bi];
            if (a.start > b.end) bi ++;
            else if (b.start > a.end) ai ++;
            else {
                ans.add (new Interval(Math.max (a.start, b.start), Math.min (a.end, b.end)));
                if (a.end > b.end) bi ++; else ai ++;
            }
        }

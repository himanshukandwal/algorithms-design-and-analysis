 ...
 * @author Hxkandwal
 */
public class EmployeeImportance extends AbstractCustomTestRunner {
    ...
    public int _getImportance(List<Employee> employees, int id) {
    ...
    public int dfs(Map<Integer, Employee> map, int id) {
        ...
        for (Integer s : e.subordinates) ans += dfs(map, s);
        return ans;
    }

    // linear scanning using the input list.
    public int _getImportanceOther(List<Employee> employees, int id) {
        int ans = 0;
        for (Employee e : employees) {
            if (e.id == id) {
                ans += e.importance;
                for (Integer s : e.subordinates) ans += _getImportanceOther(employees, s);
                break;
            }
        }
        return ans;

...
public class NextClosestTime extends AbstractCustomTestRunner {
    ...
    public String nextClosestTime(String time) {
        time = time.replace(":", "");
        int [] limits = new int [] { 2, 9, 5, 9 }; // limits
        int[] sorted = new int [4];

        for (int idx = 0, jdx = 0; idx < time.length(); idx ++) sorted [jdx ++] = (time.charAt (idx) - '0');
        Arrays.sort (sorted);
        ...
        for (; idx >= 0; idx --) {
            ...
            int sIdx = 0;
            while (sIdx < sorted.length) if (sorted [sIdx] <= num) sIdx ++; else break;

            if (sIdx < sorted.length && limits [idx] >= sorted [sIdx]) {
                ans = time.substring(0, idx) + String.valueOf(sorted [sIdx]);
                break;
            }
        }
        if (idx >= 1 && Integer.valueOf(ans.substring(0, 2)) > 23) { ans = ans.substring (0, 1); idx = 0; }
        for (idx = idx + 1; idx < time.length(); idx ++) ans += String.valueOf(sorted [0]);

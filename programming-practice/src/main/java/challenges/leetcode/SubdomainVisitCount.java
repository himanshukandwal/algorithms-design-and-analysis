 ...
 *
 *  Explanation: We only have one website domain: "discuss.leetcode.com". As discussed above, the subdomain "leetcode.com" and "com" will also be visited.
 ...
 *
 *  Explanation: We will visit "google.mail.com" 900 times, "yahoo.com" 50 times, "intel.mail.com" once and "wiki.org" 5 times. For the subdomains, we will visit
 ...
 *
 */
public class SubdomainVisitCount extends AbstractCustomTestRunner {

    public List<String> subdomainVisits(String[] cpdomains) {
        if (cpdomains.length == 0) return Arrays.asList();
        List<String> ans = new ArrayList<>();
        Map<String, Integer> map = new HashMap<>();
        for (String d : cpdomains) {
            int count = Integer.valueOf(d.split(" ")[0]);
            String sd = d.split(" ")[1];

            map.put(sd, map.getOrDefault(sd, 0) + count);
            for (int idx = 0; idx < sd.length(); idx ++)
                if (sd.charAt(idx) == '.') map.put(sd.substring(idx + 1), map.getOrDefault(sd.substring(idx + 1), 0) + count);
        }

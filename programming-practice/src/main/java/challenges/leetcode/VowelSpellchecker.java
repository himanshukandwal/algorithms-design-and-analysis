 ...
 * @author Hxkandwal
 */
public class VowelSpellchecker extends AbstractCustomTestRunner {

    public String[] spellchecker(String[] wordlist, String[] queries) {
        Set<String> words = new HashSet<>(Arrays.asList(wordlist));
        Map<String, String> lowered = new HashMap<>(), replaced = new HashMap<>();
        for (String w : wordlist) {
            String lower = w.toLowerCase();
            String replace = lower.replaceAll("[aeiou]", "#");

            lowered.putIfAbsent(lower, w);
            replaced.putIfAbsent(replace, w);
        }

        for (int idx = 0; idx < queries.length; idx ++) {
            if (words.contains(queries[idx])) continue;
            String lower = queries[idx].toLowerCase();
            String replace = lower.replaceAll("[aeiou]", "#");

            queries [idx] = lowered.getOrDefault(lower, replaced.getOrDefault(replace, ""));
        }
        return queries;
    }

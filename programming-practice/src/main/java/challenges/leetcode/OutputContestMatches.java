 ...
 *      Input: 4
 *      Output: ((1,4),(2,3))
 *      Explanation: In the first round, we pair the team 1 and 4, the team 2 and 3 together, as we need to make the strong team and weak team together.
 *                   And we got (1,4),(2,3).
 ...
 * @author Hxkandwal
 */
public class OutputContestMatches extends AbstractCustomTestRunner {

    public String _findContestMatch(int n) {
        String[] t = new String [n];
        for (int idx = 1; idx <= n; idx ++) t [idx - 1] = String.valueOf(idx);
        while (n > 0) {
            int s = 0, e = n -1;
            while (s < e) {
                t [s] = "(" + t [s] + "," + t [e] + ")";

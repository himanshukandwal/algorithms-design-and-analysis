 ...
 * @author Hxkandwal
 */
public class SumOfEvenNumbersAfterQueries extends AbstractCustomTestRunner {

    public int[] _sumEvenAfterQueries(int[] A, int[][] queries) {
        int [] ans = new int [queries.length];
        int sum = 0;
        for (int v : A) if (v % 2 == 0) sum += v;
        for (int idx = 0; idx < queries.length; idx ++) {
            int incr = queries [idx][0], index = queries [idx][1];
            int oldVal = A [index], newVal = oldVal + incr;
